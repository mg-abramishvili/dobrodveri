        ...
        <div class="row align-items-center">
            <div class="col-12 col-md-6">
                <h1>
                    ...
                    <template v-if="$route.params.id">{{ innerdecor.name }}</template>
                    <template v-else>Новая внутренняя отделка</template>
                </h1>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        save() {
            ...
            if(this.$route.params.id) {
                axios.put(`/_admin/innerdecor/${this.$route.params.id}/update`, postData)
                .then(response => {
            ...
            if(!this.$route.params.id) {
                axios.post(`/_admin/innerdecors`, postData)
                .then(response => {
                    this.views.saveButton = true
                    this.$router.push({ name: 'InnerDecors' })
                })
