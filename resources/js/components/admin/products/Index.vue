                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="product in productsFiltered" :key="product.id">
                            <td>{{ product.name }}</td>
                            <td class="text-end">
                                <router-link :to="{ name: 'ProductMaster', params: {id: product.id} }" class="btn btn-sm btn-outline-primary">
                                ...
                                <button @click="deactivate(product.id)" class="btn btn-sm btn-outline-danger ms-1">
    ...
    methods: {
        ...
        deactivate(id) {
            if (confirm("Товар будет перенесён в раздел Удаленные. Продолжить?")) {
                axios.put(`/_admin/product/${id}/deactivate`)
                .then(response => {
                    this.loadProducts()
                })
                .catch(errors => {
