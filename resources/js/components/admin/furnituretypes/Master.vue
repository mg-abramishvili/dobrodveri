    ...
    <div v-if="!views.loading" class="subheader w-100">
        <div class="row align-items-center">
            <div class="col-12 col-md-6">
                <h1>
                    <router-link :to="{ name: 'FurnitureTypes' }">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-left-short" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M12 8a.5.5 0 0 1-.5.5H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5a.5.5 0 0 1 .5.5z"/>
                        </svg>
                    </router-link>

                    <template v-if="$route.params.id">{{ furnituretype.name }}</template>
                    <template v-else>Новый тип фурнитуры</template>
                </h1>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        loadFurnitureType() {
            axios.get(`/_admin/furnituretype/${this.$route.params.id}`)
            .then(response => {
                this.furnituretype = response.data
        ...
        save() {
            ...
            if(this.$route.params.id) {
                axios.put(`/_admin/furnituretype/${this.$route.params.id}/update`, postData)
                .then(response => {
                    this.views.saveButton = true
                    this.$router.push({ name: 'FurnitureTypes' })
                })
                .catch(errors => {
                    this.views.saveButton = true
            ...
            if(!this.$route.params.id) {
                axios.post(`/_admin/furnituretypes`, postData)
                .then(response => {
                    this.views.saveButton = true
                    this.$router.push({ name: 'FurnitureTypes' })
                })
